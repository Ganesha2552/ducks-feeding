...
router.post("/create", (req, res) => {
  ...
  if (!token)
    return res
      .status(401)
      .send({ auth: false, message: "No token provided. Unauthorised" });
  ...
  jwt.verify(token, keys.secretOrKey, function (err, decoded) {
    ...
    User.findById(decoded.id).then((user) => {
      if (!user) {
        return res
          .status(401)
          .json({ auth: false, message: "Unauthorised User" });
      }
    });
    ...
    User.findById(decoded.id).then((user) => {
      ...
      if (isAdmin) {
        ...
        Feeding.find({}, function (err, records) {
          ...
          .lean()
          .select("-_id -__v -autoschedule_enable");
    ...
    User.findById(decoded.id).then((user) => {
      ...
      if (feedingIds.indexOf(id) == -1) {
        return res
          ...
          .json({
            auth: false,
            message: "Not a valid record! unable to delete",
          });
      }
      Feeding.findByIdAndDelete(id, function (err, docs) {
        if (err) {
          return res
            ...
            .send({
              auth: false,
              message: "Failed to delete.Try after sometime.",
            });
        }
          ...
          function (err, model) {
            if (err) {
              return res
                ...
                .send({
                  auth: false,
                  message: "1Failed to delete.Try after sometime.",
                });
            }
            SchedulerTask.findOneAndDelete({
              job_data: decoded.id + "##" + id,
            }).then((output) => console.log("scheduler deleted ," + output));
